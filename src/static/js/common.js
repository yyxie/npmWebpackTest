...
common.getAesstExclInfo = function () {
  var staff = common.getStaff()
  var result = {
    exclusiveOrgId: staff.orgId,  // 当前登录的orgId
    exclusiveOrgType: "-1",       // 当前用户的orgType
    inclusiveOrgType: "-1",       // 需要查询的运营商的orgType

  }
  if(staff.orgTypes && staff.orgTypes.length) {

    for (var i = 0; i < staff.orgTypes.length; i++) {

       // PLATFORM(1, "平台"), APT_BUSINESS(2, "运营商"), INVEST_BUSINESS(4, "资产运营商");

      if(staff.orgTypes[i].orgId == result.exclusiveOrgId) {
        result.exclusiveOrgType = staff.orgTypes[i].orgType
        if (staff.orgTypes[i].orgType == "1126001") {  // 公寓运营商，则显示电费运营商选择框
          //
          result.exclusiveOrgType = 2;
          result.inclusiveOrgType = 4;
        } else if (result.exclusiveOrgType == "1126006") {  // 资产运营商，则显示公寓运营商选择框
          result.exclusiveOrgType = 4;
          result.inclusiveOrgType = 2;
        }
        break;
      }
...
common.getElectricExclInfo = function () {
  var staff = common.getStaff()
  var result = {
    exclusiveOrgId: staff.orgId,  // 当前登录的orgId
    exclusiveOrgType: "-1",       // 当前用户的orgType
  ...
  if(staff.orgTypes && staff.orgTypes.length) {
    ...
    for (var i = 0; i < staff.orgTypes.length; i++) {
       ...
       // PLATFORM(1, "平台"), APT_BUSINESS(2, "运营商"), INVEST_BUSINESS(3, "电费运营商");
      ...
      if(staff.orgTypes[i].orgId == result.exclusiveOrgId) {
        ...
        if (staff.orgTypes[i].orgType == "1126001") {  // 公寓运营商，则显示电费运营商选择框
          //
          result.exclusiveOrgType = 2;
          result.inclusiveOrgType = 3;
        } else if (result.exclusiveOrgType == "1126005") {  // 电费运营商，则显示公寓运营商选择框
          result.exclusiveOrgType = 3;
          result.inclusiveOrgType = 2;
        }
        break;
      }
...
common.getAssetExclInfo = function () {
  var staff = common.getStaff()
  var result = {
    exclusiveOrgId: staff.orgId,  // 当前登录的orgId
    exclusiveOrgType: "-1",       // 当前用户的orgType
    ...
    for (var i = 0; i < staff.orgTypes.length; i++) {
      ...
      if(staff.orgTypes[i].orgId == result.exclusiveOrgId) {
        ...
        if (staff.orgTypes[i].orgType == "1126001") {  // 公寓运营商，则显示电费运营商选择框
          ...
          result.exclusiveOrgType = 2;
          result.inclusiveOrgType = 4;
        } else if (result.exclusiveOrgType == "1126006") {  // 资产运营商
          result.exclusiveOrgType = 4;
          result.inclusiveOrgType = 2;
        }
        break;
...
common.getPostMan = function(data){
  var str = "";
